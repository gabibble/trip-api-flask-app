...
def profile():
    ...
    try:
        if request.method == "POST": #and form.validate_on_submit():
            ...
            print(trip_name, city, state, country, people, accommodation, trip_length, trip_date) #, user_token=user_token)
            # instantiiating trip(comment this put until u create user class and DB first!)
            # trip = Trip(trip_name, city, state, country, people, accommodation, trip_length, trip_date)
            # db.session.add(trip)
            # db.session.commit()
            #
